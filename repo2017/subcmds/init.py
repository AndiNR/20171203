...
"""
  ...
  def _ConfigureTemplate(self):
    src_template = os.path.abspath('.repo/repo/hooks/commit_template')
    dst_template = os.path.expanduser('~/.commit_template')
    if os.path.isfile(src_template):
      if os.path.islink(dst_template):
        try:
          os.remove(dst_template)
        except OSError as e:
            print('fatal: %s' % str(e))
            print('remove old commit_template error.')
      try:
        open(dst_template, "wb").write(open(src_template, "rb").read())
      except OSError as e:
         print('fatal: %s' % str(e))
         print('write commit_template error.')
      if os.path.isfile(dst_template):
        gt = self.manifest.globalConfig
        gt.SetString('commit.template',dst_template)
